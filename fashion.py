...
class ThemeAnalysisAgent(BaseAgent):
    """
    Agent 1: 负责分析时尚主题
    任务：分析给定主题（如Givenchy作为新时尚系列"I'm Your Mirror"的主题）
    输入：Design target prompt 和 Theme
    输出：Theme analysis（包含Overall Theme, Inspiration, Key aspects of the theme）
    """
    def __init__(self, model="gpt-4o-mini", notes=None, max_steps=100, openai_api_key=None):
    ...
    def command_descriptions(self, phase):
        ...
        if phase == "theme analysis":
            return (
                ...
                "When you have completed the theme analysis, submit it using: ```THEME_ANALYSIS\ntheme analysis here\n```\n"
                "The output must follow this structure:\n\n"
                "**Theme:**\n"
                "[Collection title and overview of the theme in paragraph format.]\n\n"
                "**Inspiration:**\n"
                "[Information about the designer, inspiration source, and how it translates into the collection. Write in paragraph form.]\n\n"
                "**Key aspects of the theme:**\n"
                "[Break down the theme into key aspects. For each aspect, use a bold heading (e.g., **Aspect Name:**) followed by a descriptive paragraph.]\n\n"
                ...
                "where COMMAND is the specific command you want to run (e.g. THEME_ANALYSIS, DIALOGUE).\n"
            )
        return ""
...
class LookDescriptionAgent(BaseAgent):
    ...
    def phase_prompt(self, phase):
        ...
        if phase == "look description generation":
            phase_str = (
                "Generate ONE detailed look description based on the candidate design elements.\n"
                "Focus on visual aspects; weave color into each section naturally (no separate palette block).\n"
                "Detail & transitions: Describe each part with concrete detail (materials, construction, proportions). Use clear transitions between sections—e.g. how the outerwear meets the foundation, how details echo the main pieces, how footwear completes the line. The reader should see how the look builds from layer to layer.\n"
                "IMPORTANT: Ensure the look description is reasonable and feasible. Consider technical construction feasibility, material compatibility, proportion balance, and practical wearability. All garment components should work together logically and create a cohesive, wearable look.\n"
                "Use the exact look number specified in the research topic.\n"
            )
        return phase_str
    ...
    def command_descriptions(self, phase):
        ...
        if phase == "look description generation":
            return (
                ...
                "The output must follow this structure (generate ONLY ONE look description; information-dense, with rich detail and smooth transitions between sections):\n\n"
                ...
                "1. The Outerwear: [Name/Type]\n"
                "[Write a detailed paragraph: silhouette, material, construction, and color. Describe how it sits on the body and leads into the layer beneath.]\n"
                "    *   Key Structure: [2-4 short phrases: shoulder/lapel/closure/length]\n"
                "    *   Material & Finish: [fabric + surface + hardware]\n\n"
                "2. The Foundation: [Core Garment Set]\n"
                "[Write a detailed paragraph: top and bottom (or dress), how they support the outerwear and create the line. Include color and how this layer transitions to details.]\n"
                "    *   Top: [one line]\n"
                "    *   Bottom: [one line]\n"
                "    *   Fit & Line: [one line]\n\n"
                "3. The Details: Materiality & Hardware\n"
                "[Write a detailed paragraph: 2-4 key details, with material and color. Explain how these details echo or contrast with the main pieces.]\n"
                "    *   Hardware Focus: [one line]\n"
                "    *   Waist Treatment: [one line]\n\n"
                "4. The Finish: Footwear & Stance\n"
                "[Write a detailed paragraph: footwear, stance, and movement. Describe how the shoe completes the silhouette and ties the look together.]\n"
                "    *   Shoe: [one line]\n\n"
                "Do NOT include a separate Palette section; weave color into each section above.\n\n"
                "You can only use a SINGLE command per inference turn. Do not use more than one command per inference.\n"
                "When performing a command, make sure to include the three ticks (```) at the top and bottom ```COMMAND\ntext\n``` "
    ...
    def example_command(self, phase):
        ...
        return (
            ...
            "Please generate female models and the matching clothing for them.\n\n"
            "Look textual description\n\n"
            "Look 01: The Sentinel's Departure\n\n"
            "1. The Outerwear: The Architectural Greatcoat\n"
            "An ankle-length, double-breasted greatcoat in heavyweight charcoal cavalry twill, cut straight to create a severe column from shoulder to hem; matte-black horn buttons and sharp peak lapels keep the mood disciplined and modern. The coat's clean line and weight establish the silhouette that the foundation layer will extend.\n"
            "    *   Key Structure: pagoda shoulder; oversized peak lapel; 6x3 double-breasted; ankle length\n"
            "    *   Material & Finish: charcoal cavalry twill + matte horn buttons + pressed edges\n\n"
            "2. The Foundation: Columnar Shirt + Wide-Leg Trouser\n"
            "A stark white high-collar poplin shirt locked to the throat sits under the coat; high-waisted wide-leg charcoal wool trousers continue the vertical line from the hem. The clean shirt front and sharp trouser crease maintain a strict column with controlled movement, so that the outerwear and foundation read as one continuous form.\n"
            "    *   Top: white poplin, high stiff collar, hidden placket\n"
            "    *   Bottom: charcoal wool, high waist, wide leg, sharp pressed crease\n"
            "    *   Fit & Line: uninterrupted vertical line; grounded stance from wide hem sweep\n\n"
            "3. The Details: Materiality & Hardware\n"
            "Two signals of tailoring credibility: functional surgeon’s cuffs and a minimal matte-black belt that defines the waist without breaking the column.\n"
            "    *   Hardware Focus: surgeon’s cuffs with small matte horn buttons\n"
            "    *   Waist Treatment: matte-black leather belt, worn high over the coat\n\n"
            "4. The Finish: Footwear & Stance\n"
            "Polished black oxfords with a double-stacked sole add weight; the model moves with a deliberate, even stride that matches the coat’s strict geometry.\n"
            "    *   Shoe: black box-calf oxford, double-stacked sole\n"
            "```\n"
        )

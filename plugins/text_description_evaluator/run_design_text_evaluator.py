...
def _run_adaptive_file(
    ...
    del report_dir, max_rounds, min_score_improvement, min_quality_improvement
    gate_cfg = {}
    ...
    gate_arg = gate_cfg if gate_cfg else None
    text = input_path.read_text(encoding="utf-8").strip()
    evaluation_result = evaluator.evaluate_text(text, source_name=input_path.name, gate_config=gate_arg)
    should_flag, decision = _should_optimize_adaptively(
    ...
    return {
        ...
        "evaluation_result": evaluation_result,
        "optimization_result": {"note": "multi-round optimize removed; no auto rewrite"} if should_flag else None,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="adaptive",
        help="evaluate：仅评判；optimize / adaptive：已废弃多轮改写，等同于仅评判（adaptive 仍输出是否「本会触发改写」的决策信息）。",
    )
    parser.add_argument(
        "--input",
        required=True,
...
def main() -> None:
    ...
    def _gate_kw() -> Dict:
        gc = {}
        if args.score_gate_min is not None:
            gc["score_gate_min"] = args.score_gate_min
        if args.penalty_gate_max is not None:
            gc["penalty_gate_max"] = args.penalty_gate_max
    ...
    if input_path.is_dir():
        if args.mode == "adaptive":
            ...
            gkw = _gate_kw() or None
            result = {}
            for txt_file in sorted(input_path.glob("*.txt")):
                text = txt_file.read_text(encoding="utf-8").strip()
                result[txt_file.name] = evaluator.evaluate_text(
                    text, source_name=txt_file.name, gate_config=gkw
                )
        else:
        ...
        if args.mode == "adaptive":
            ...
            gkw = _gate_kw() or None
            text = input_path.read_text(encoding="utf-8").strip()
            result = evaluator.evaluate_text(text, source_name=input_path.name, gate_config=gkw)
        else:

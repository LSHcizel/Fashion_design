...
from plugins.local_llm import (
    ...
    resolve_workflow_llm_backend,
    workflow_vision_model_name,
)
...
def parse_sub_themes_from_theme_analysis(theme_analysis_text: str) -> list[str]:
    """
    从 ThemeAnalysisAgent 输出中提取「章节子主题」列表。
    约定：子主题仅来自 **Sub-themes for chapter development:** 段落的加粗标题；
    **Key aspects of the theme:** 为综合性分析，不参与子主题解析。
    """
    if not theme_analysis_text:
        return []

    if SUB_THEMES_MARKER not in theme_analysis_text:
        return []

    tail = theme_analysis_text.split(SUB_THEMES_MARKER, 1)[1]
    # 支持 ``**Title:**`` 与 ``**1. Title**`` 两种子主题标题格式
    matches = re.findall(r"\*\*([^*]+?)(?::)?\*\*", tail)
    sub_themes = []
    seen = set()
    for m in matches:
        name = re.sub(r"^\d+\.\s*", "", m.strip()).strip()
...
def resolve_chapter_sub_themes(theme_analysis_text: str, num_chapters: int) -> list[str]:
    """
    按 num_chapters 确定章节子主题数量（配置优先）。
    解析失败或条数不足时用 Sub-Theme N 占位补齐；条数过多则截断。
    """
    n = max(1, int(num_chapters or 1))
    parsed = parse_sub_themes_from_theme_analysis(theme_analysis_text or "")
    if len(parsed) >= n:
        return parsed[:n]
    names = list(parsed)
    while len(names) < n:
...
class FashionWorkflow:
    def __init__(
        ...
        @param description: (str) 主题补充说明（可选，仅主题分析阶段使用）
        ...
        if self.evaluator_config.enabled:
            self.text_evaluator = build_workflow_text_evaluator(
    ...
    def _regenerate_eliminated_chapters(self, chapter_names: list):
        ...
        重新生成被淘汰的 chapter
        """
        sub_themes = resolve_chapter_sub_themes(
    ...
    def theme_analysis(self):
        ...
        self.theme_analyzer.description = self.description or ""
        self.theme_analyzer.num_chapters = self.num_chapters

        research_topic = f"Design Target: {self.design_target_prompt}\nTheme: {self.theme}\nNum chapters (sub-themes): {self.num_chapters}"
        ...
        for _i in range(max_tries):
            ...
            if "```DESCRIPTION_KEY_ELEMENTS" in resp:
                key_elements = extract_prompt(resp, "DESCRIPTION_KEY_ELEMENTS")
                self.theme_analyzer.description_key_elements = key_elements
                if self.workflow_dir and key_elements.strip():
                    key_path = os.path.join(self.workflow_dir, "description_key_elements.txt")
                    with open(key_path, "w", encoding="utf-8") as f:
                        f.write(key_elements.strip() + "\n")
                    if self.verbose:
                        print(f"Saved description key elements → {key_path}")
            ...
            if "```THEME_ANALYSIS" in resp:
                ...
                self.theme_analyzer.sub_themes = resolve_chapter_sub_themes(
                    theme_analysis, self.num_chapters
                )
    ...
    def process_chapters_sequentially(
        ...
        skip_collection_reflection: bool = False,
    ):
        """按顺序处理每个章节：每次生成1个章节（=1个子主题），处理完后再生成下一个"""
        chapter_names = resolve_chapter_sub_themes(
            getattr(self.theme_analyzer, "theme_analysis", "") or "",
            self.num_chapters,
        )
        self.theme_analyzer.sub_themes = chapter_names
        chapter_from = max(1, int(chapter_from or 1))
        if chapter_to is None:
            chapter_to = len(chapter_names)
        else:
            chapter_to = min(int(chapter_to), len(chapter_names))
            ...
            if export_root:
                ...
                paths = write_look_evaluation_artifacts(
                    ...
                    extra_summary={
                        ...
                        "source_txt": str(Path(look_txt_path).name) if look_txt_path else None,
                    },
                )
                result["report_path"] = paths.get("report_md")
                result["evaluation_json_path"] = paths.get("evaluation_json")
                if self.verbose:
                    print(f"  [TextEval] 明细已写入 {Path(paths['report_md']).parent}")
    ...
    def look_description_generation_for_group(self, group_dir=None):
        ...
        for look_idx in range(num_looks):
            ...
            # 仅清除 inference 历史；保留 look_descriptions 供同章后续 look 差异化
            self.look_stylist.reset()
...
class TextEvaluatorConfig:
    enabled: bool = False
    # total / quality 下两项历史上用于 adaptive optimize，主流程已改为仅认双门限；保留字段供兼容或未引用路径。
...
def parse_yaml(yaml_file_loc):
    ...
    if "description" in config_data:
        parser.description = config_data["description"]
    else:
        parser.description = ""
    ...
    if "llm-backend" in config_data:
        parser.llm_backend = resolve_workflow_llm_backend(config_data["llm-backend"])
    else:
        parser.llm_backend = resolve_workflow_llm_backend(DEFAULT_LLM_BACKBONE)
...
if __name__ == "__main__":
    ...
    if human_mode:
        desc_input = input("请输入主题补充说明 description（可直接回车跳过）: ").strip()
        if desc_input:
    ...
    if vision_model:
        agent_models["single look reflection"] = vision_model
        agent_models["chapter image reduction"] = vision_model
        agent_models["collection reflection"] = vision_model
